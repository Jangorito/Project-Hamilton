...
- **Interaction effect:** v21b's traction constraint is ~5× more damaging for SBR than ETK. The SBR4 has a narrow, sharp handling envelope (snap oversteer, low polar moment). Under V1 it found an aggressive policy exploiting this — 2419.9 m best checkpoint. Under v21b's physics-shaped speed target the SBR's cornering style continuously triggered the traction penalty, disrupting the existing policy without producing a cautious replacement: Condition D still terminated by `damage` (1030.2 m best, 549.6 m final). The large D deterministic/stochastic gap (753.9 m) confirms the policy sits on the traction boundary — stochastic sampling occasionally stays safe, the deterministic mean crashes. The ETK K-Series, with more progressive oversteer and greater tolerance for lateral slip, found the same `allowed_aggression=1.0` closer to its natural operating range, so degraded less.
...
*RQ2:* Strong vehicle × reward interaction. v21b disrupted the SBR's aggressive V1 policy without producing a cautious replacement — Condition D still terminates by `damage`, not max_episode_steps. The deterministic/stochastic gap (753.9 m for D) confirms the policy is unstable at the traction boundary, not conservatively safe. The ETK degraded less because its more progressive dynamics were closer to the `allowed_aggression=1.0` budget. This is not a failure of v21b as a design — it reveals that `allowed_aggression` is vehicle-specific. A future condition (v21c, `allowed_aggression=1.15`) would test whether a higher budget recovers SBR performance.
