...
The SBR4 has a narrow, sharp handling envelope (snap oversteer, low polar moment). Under V1 reward, SBR found an aggressive policy that exploited this — achieving 2419.9 m at its best checkpoint. Under v21b's physics-shaped speed target (`v_target = sqrt(8.0/κ)`), with `allowed_aggression=1.0`, the SBR's aggressive cornering style continuously triggered the traction penalty. Rather than producing a cautious, conservative policy, v21b disrupted the SBR's existing policy without giving it a reliable traction-aware replacement: Condition D still terminated by `damage` (1030.2 m best checkpoint, 549.6 m final). The large deterministic/stochastic gap for D (753.9 m) is consistent with a policy sitting on the traction boundary — stochastic sampling occasionally stays safe, but the deterministic mean crashes. The ETK K-Series, with more progressive oversteer and greater tolerance for lateral slip, found v21b less constraining: the same `allowed_aggression=1.0` was closer to its natural operating range, which is why the ETK degraded less (−276.8 m) than the SBR (−1389.7 m).
